---
title: "precipitation projections"
...
reference_period <- df %>%
  ...
  summarise(Reference_Avg = mean(Avg_Rain, na.rm = TRUE)) %>%
  pull(Reference_Avg)
...
write.csv(projections,file="rainfall_projections.csv" )
```

```{r}
p <- ggplot() +
  geom_line(data = df, aes(x = Year, y = Avg_Rain), color = "black", size = 1) +
  labs(
    title = paste(mapped_location, "- Precipitation Senario Projections"),
    x = "Year",
    y = "Precipitation (mm)"
  ) +
  theme_minimal()
...
```{r}
ggsave(filename = paste0("rainfall_projections_", location, ".png"),
       plot = p,
       width = 10,
       height = 6)
```
